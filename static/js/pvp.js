...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateInfoPanel(state) {
        ...
        const playerTurn = state.currentPlayer === 'white' ? 'Oyuncu 1 (Beyaz)' : 'Oyuncu 2 (Siyah)';
        turnInfoElement.textContent = `Sıra: ${playerTurn}`;
        gameStatusElement.textContent = (validMoves.length > 0 && validMoves[0].captured.length > 0) ? 'Zorunlu hamle var!' : '';
    }
    ...
    async function sendMove(move) {
        try {
            const response = await fetch('/pvp_move', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(move), });
            if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    ...
    function handleGameOver(state) {
        ...
        if (state.winner === 'draw') { message = "Gayyım! Oyun berabere."; } else { const winnerName = state.winner === 'white' ? 'Oyuncu 1 (Beyaz)' : 'Oyuncu 2 (Siyah)'; message = `${winnerName} kazandı!`; }
        showModal(message);
    }
